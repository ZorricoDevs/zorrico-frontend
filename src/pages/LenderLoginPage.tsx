...
const LenderLoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user.role !== 'lender') {
        setError('Access denied. Lender account required.');
        setIsLoading(false);
        return;
      }
      ...
      if (err.response?.data?.error) {
        ...
        setError('Invalid lender credentials. Please check your email and password.');
      }
    } finally {
      setIsLoading(false);
    }
        ...
        <CardContent sx={{ p: 4 }}>
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            ...
            <Typography variant="body2" sx={{ color: '#666' }}>
              Access your lending portal and manage loans
            </Typography>
          </Box>
          ...
          <Box component="form" onSubmit={handleLogin}>
            <Stack spacing={3}>
              ...
              >
                {isLoading ? 'Authenticating...' : 'Login as Lender'}
              </Button>
            </Stack>
          </Box>
          ...
          <Box sx={{ mt: 4, textAlign: 'center' }}>
            <Typography variant="caption" sx={{ color: '#666' }}>
              Institutional lender? Contact our partnership team
            </Typography>
            <br />
            <Typography variant="caption" sx={{ color: '#666' }}>
              All lender accounts require verification and approval
            </Typography>
          </Box>
        </CardContent>
      </Card>
